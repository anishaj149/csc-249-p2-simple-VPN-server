...
def encode_message(message):
    ...
    # adding the server ip and port information
    return message + ":" + str(SERVER_IP) + ":"+ str(SERVER_PORT)
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    print(f"connection established, sending message '{encoded_message}'")
    s.sendall(bytes(encoded_message, 'utf-8'))
